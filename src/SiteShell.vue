...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('clothing.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('clothing.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('clothing.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
